...
public class Enneagram {
    ...
    public void processResults() throws FileNotFoundException {
        ...
        while(scan.hasNextLine()) {
            ...
            if(studentData.length <= 37){
                scan.close();
                throw new InvalidEnneagramException("Invalid number of answers for the current student.");
            }

            boolean isValid = validateAnswers(new ArrayList<>(Arrays.asList(studentData)));

            if(isValid){
                tests.add(new ArrayList<>(Arrays.asList(studentData)));
            }
            else{
                scan.close();
                throw new InvalidEnneagramException("Invalid answer in student data.");
            }
        }
    ...
    private boolean validateAnswers(ArrayList<String> oneStudent) {
        ...
        for (int i = 1; i < oneStudent.size(); i++) {
            String answer = oneStudent.get(i);
            if (validAnswers(answer, i - 1)) {
                throw new InvalidEnneagramException("Invalid answer for student: " + i);
            }
        }
